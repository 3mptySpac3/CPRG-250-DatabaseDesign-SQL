...
/* 
SELECT table_name FROM user_tables WHERE table_name = 'TMB_CATEGORY';
*/
...
SPOOL C:\TimberSpool\Tmber_Project_Output9.txt --- Location of output file---
...
create table tmb_supplier(
  ...
  constraint sys_tmb_supplier_province_ck
    check ((country <> 'Canada') or( province in ('AB', 'BC', 'SK', 'MB', 'ON', 'QC', 'NB', 'NS', 'NL', 'NT', 'NU', 'PE', 'YT')))
);
...
create table tmb_shipping_rate(
  ...
  weight_kg varchar2(50) not null
);
...
create table tmb_customers(
  ...
  email varchar(60) not null constraint sys_tmb_customers_email_ck
    check(email like '%@%'),
...
create table tmb_order_items(
  ...
  constraint sys_tmb_order_item_pk primary key(order_number,product_number)
  );
...
create table tmb_reviews(
  review_number number constraint sys_tmb_reviews_pk primary key,
  customer_number number constraint sys_tmb_reviews_fk
...
---alter tables still hold values true to main table---
  alter table tmb_reviews
    add rating number(1,1) not null constraint sys_tmb_reviews_rating_ck
      check (rating  >=1 and rating <=5 );
  alter table tmb_reviews
    add comments varchar2(200);
  alter table tmb_reviews
    add price number(8,2) not null constraint sys_tmb_reviews_price_ck
      check (price >= 0);
  alter table tmb_reviews
    add waight number(4,1) not null constraint sys_tmb_reviews_waight_ck
      check (waight >= 0);
  alter table tmb_reviews
    add tax_exempt_flag number(1) null constraint sys_tmb_reviews_tax_exempt_flag_ck
      check (tax_exempt_flag in (0,1) or tax_exempt_flag is null);
